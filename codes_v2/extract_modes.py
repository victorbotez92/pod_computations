...
def main_extract_modes(inputs):
    ...
    for num_mF,mF in enumerate(inputs.list_modes[inputs.rank_fourier::inputs.nb_proc_in_fourier]):
        ...
        for a in range(inputs.rank_axis,2,inputs.nb_proc_in_axis):
            ...
            for i,path_to_data in enumerate(inputs.paths_to_data):
                ...
                for individual_path_to_data in path_to_data:
                    ...
                    new_data = import_data(inputs,mF,[individual_path_to_data])[:, a::2, :] #shape n d t
                    new_data = nodes_to_gauss(new_data, inputs)
                    write_job_output(inputs,f'      Successfully imported {individual_path_to_data}')
                    ...
                    if inputs.should_we_add_mesh_symmetry:
                        ...
                        apply_mesh_sym(inputs, sym_data, axis, mF)
