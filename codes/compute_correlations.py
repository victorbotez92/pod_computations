...
from compute_renormalizations import renormalize_and_rm_mean_field, apply_mesh_sym
###############################
...
#======================================
    ...
    def compute_correlation(self, i, j, inputs, global_coeff=1, left_matrix=None, right_matrix=None):#, symmetrize = False):   
        ...
        if right_matrix is None:
            right_matrix = np.copy(left_matrix)

        output = einsum(left_matrix, global_coeff*right_matrix*inputs.W.reshape(inputs.W.shape[0],1,1), "n D T, n D T_prime -> T T_prime")
...
def core_correlation_matrix_by_blocks(inputs,mF,axis,consider_crossed_correlations=False):
    ...
    for i in range(nb_paths):
        ...
        raw_first_matrix = import_data(inputs,mF,inputs.paths_to_data[i],list_T=inputs.list_T_per_individual_path[i]) # shape n d t
        first_matrix = raw_first_matrix[:, a::2, :]
        del raw_first_matrix
        gc.collect()

        renormalize_and_rm_mean_field(inputs, first_matrix, mF, axis)
        first_matrix = nodes_to_gauss(first_matrix, inputs)
        write_job_output(inputs,f"      In POD on Fourier {i} => {inputs.paths_to_data[i]} imported as left matrix")

        # print_memory_usage(inputs, tag=f"After importing left matrix of shape {first_matrix.shape}")
...
###################### ============================================================
        ...
        if inputs.should_we_add_mesh_symmetry:
            ...
            second_matrix = np.copy(first_matrix)
            apply_mesh_sym(inputs, second_matrix, inputs.tab_pairs_gauss, None, None, opt_coeff=1)
            all_blocks.compute_correlation(i, i+nb_paths, inputs, global_coeff=sym_coeff, left_matrix=first_matrix, right_matrix=second_matrix)#,weights = weight_sym_on_right)
    ...
    ############### ==============================================================
        ...
        for j in range(i+1,nb_paths):
            ...
            second_matrix = import_data(inputs,mF,inputs.paths_to_data[j],list_T=inputs.list_T_per_individual_path[j])[:, a::2, :] #shape n d t
            renormalize_and_rm_mean_field(inputs, second_matrix, mF, axis)
            second_matrix = nodes_to_gauss(second_matrix, inputs)
            write_job_output(inputs,f"          In POD on Fourier {j} => {inputs.paths_to_data[j]} imported as right matrix")
    ...
    ############### ==============================================================
        if consider_crossed_correlations:
            for j in range(nb_paths):
                ...
                second_matrix = import_data(inputs,mF,inputs.paths_to_data[j], list_T=inputs.list_T_per_individual_path[j])[:, (1-a)::2, :] #shape t a (d n)
                renormalize_and_rm_mean_field(inputs, second_matrix, mF, counter_axis)
                second_matrix = nodes_to_gauss(second_matrix, inputs)
                write_job_output(inputs,f"          In crossed POD on Fourier {j} => {inputs.paths_to_data[j]} imported as right matrix")
