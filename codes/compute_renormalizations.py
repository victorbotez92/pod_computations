            ...
            if os.path.exists(f'{MF_output}/mF{mF}_c.npy'): # the calculations are not made if it was already done before
                ...
                if par.should_we_add_mesh_symmetry:
                    ...
                    #  mean_data = nodes_to_gauss(mean_data, par)
                     sym_data = np.copy(mean_data).reshape(mean_data.shape[0], mean_data.shape[1], 1)
                     apply_mesh_sym_at_mF(par, sym_data, par.tab_pairs_nodes, mF)
                     
                     mean_data = (mean_data + sym_data[:, :, 0])/2
                     del sym_data
                     gc.collect()
...
def apply_3D_mesh_sym(inputs, data, tab_pairs):
    data[:] = data[tab_pairs, :, :]
...
def apply_mesh_sym_at_mF(inputs, data, tab_pairs, mF):
    data[:] = data[tab_pairs, :, :]
    if inputs.type_sym == 'Rpi': #-1 for sine types
        data[:, 0::2, :] *= inputs.sym_signs.reshape(1, inputs.D, 1)
        data[:, 1::2, :] *= -inputs.sym_signs.reshape(1, inputs.D, 1)
...
def apply_mesh_sym(inputs, data, tab_pairs, axis, mF, opt_coeff=None):
    if inputs.type_sym == 'Rpi':
        if opt_coeff is None:
            apply_rpi_symmetry(inputs, data, tab_pairs, axis)
        else:
            if opt_coeff == 1:
                apply_rpi_symmetry(inputs, data, tab_pairs, "c")
            elif opt_coeff == -1:
                apply_rpi_symmetry(inputs, data, tab_pairs, "s")
            else:
                raise ValueError(f"wrong opt_coeff {opt_coeff} in apply_mesh_sym")

    elif inputs.type_sym == 'centro':
        if opt_coeff is None:
            apply_centro_symmetry(inputs, data, tab_pairs, mF)
        else:
            if opt_coeff == 1:
                apply_rpi_symmetry(inputs, data, tab_pairs, 0)
            elif opt_coeff == -1:
                apply_rpi_symmetry(inputs, data, tab_pairs, 1)
            else:
                raise ValueError(f"wrong opt_coeff {opt_coeff} in apply_mesh_sym")

def apply_rpi_symmetry(inputs, data, tab_pairs, axis):
    data[:] = data[tab_pairs,:,:]
    data *= inputs.sym_signs.reshape(1, inputs.D, 1)
    if axis == 's':
        data *= -1

def apply_centro_symmetry(inputs, data, tab_pairs, mF):  # THIS IS WRONG BECAUSE IT WILL DEPEND ON THE FOURIER MODE mF
    data[:] = data[tab_pairs,:,:]
    data *= inputs.sym_signs.reshape(1, inputs.D, 1)
    if mF%2 == 1:
        data *= -1
