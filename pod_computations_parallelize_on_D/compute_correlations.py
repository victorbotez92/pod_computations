...
def build_symmetrized_weights(par,rows,columns,WEIGHTS,WEIGHTS_with_symmetry,D = 3,axis='c'):
    if axis == 's':
        symmetry_coeff = -1
    elif axis == 'c':
        symmetry_coeff = 1
    ...
    for d in range(D//par.nb_proc_in_dimension):#(par.rank_dimension,D,par.nb_proc_in_dimension):
        if d == 0:
            adapted_rows = rows
            adapted_columns = columns
        else:
            adapted_rows = np.concatenate((adapted_rows,rows+d*len(rows)))
            adapted_columns = np.concatenate((adapted_columns,columns+d*len(columns)))
...
def core_correlation_matrix_by_blocks(par,mF,axis,field_name_in_file,for_building_symmetrized_weights=(None,None,None,None)):
    ...
    if par.should_we_add_mesh_symmetry:
            ...
            weight_sym_on_right,weight_sym_on_left,weight_sym_on_right_and_left = build_symmetrized_weights(par,rows,columns,WEIGHTS,WEIGHTS_with_symmetry,
                                                                                    D = par.D,axis=axis)
