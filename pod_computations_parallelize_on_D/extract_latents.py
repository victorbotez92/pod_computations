...
def main_extract_latents(par):   
    ...
    ############### ==============================================================
    ############### MPI_ALL_REDUCE on dimension
    ############### ==============================================================
            if par.rank_meridian == 0:
                if par.rank_dimension != 0:
                    rank_fourier,rank_axis,rank_meridian,rank_dimension,par
                    rank_to_send = invert_rank(par.rank_fourier,par.rank_axis,par.rank_meridian,0,par) # mpi_all_reduce on rank_dimension
                    par.comm.send(correlation,dest=rank_to_send)
                elif par.rank_dimension == 0:
                    for d in range(1,par.nb_proc_in_dimension):
                        rank_recv = invert_rank(par.rank_fourier,par.rank_axis,par.rank_meridian,d,par)
                        correlation += par.comm.recv(source=rank_recv)
    ...
    ############### ==============================================================
            ...
            if par.rank_meridian == 0 and par.rank_dimension == 0:
    ...
    if par.should_we_save_phys_POD and par.rank_meridian == 0 and par.rank_dimension == 0: #mpi_all_reduce on meridian already done above
