...
# @jit(forceobj=True)
def get_data(par,path_to_suite,field,mesh_type,mF,D,S,T,N,fourier_type = ['c','s']):
    N_tot = np.sum(np.array(N))
    N_slice=np.cumsum(np.array(N)//T)
    data = np.zeros(shape=(T,D,2,N_tot//T))
    for s in range(S):
        n = N[s]
        for d in range(par.rank_dimension,par.rank_dimension+D//par.nb_proc_in_dimension):
...
def import_data(par,mF,axis,raw_paths_to_data,field_name_in_file,should_we_renormalize=True): # the last parameter is set to False only when creating the normalization coefficients
    ...
    for num,raw_path_to_data in enumerate(raw_paths_to_data):
        ...
        if par.READ_FROM_SUITE:
            ...
            new_data = rearrange(new_data[:,:,par.rank_dimension:par.rank_dimension+par.D//par.nb_proc_in_dimension],"t n d -> t (d n)")[:,None,:]
        else:
            if par.D > 1:
            ...
            new_data = get_data(par,par.path_to_suites+path_to_data,# parallelization in D is dealth with in function get_data
