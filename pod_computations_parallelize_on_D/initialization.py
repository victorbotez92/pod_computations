...
list_ints = ['D','S','MF','nb_proc_in_fourier','nb_proc_in_axis','nb_proc_in_meridian','nb_proc_in_dimension']
...
nb_proc_in_dimension = par.nb_proc_in_dimension

assert (size==nb_proc_in_fourier*nb_proc_in_axis*nb_proc_in_meridian*nb_proc_in_dimension)

par.comm = comm
par.rank = rank
par.size = size

rank_axis,rank_fourier,rank_meridian,rank_dimension = indiv_ranks(par)
par.rank_axis,par.rank_fourier,par.rank_meridian,par.rank_dimension = rank_axis,rank_fourier,rank_meridian,rank_dimension
assert rank == invert_rank(par.rank_fourier,par.rank_axis,par.rank_meridian,par.rank_dimension,par)
...
if should_we_add_mesh_symmetry:
    ...
    WEIGHTS_with_symmetry = np.array([relative_signs[d]*W for d in range(rank_dimension,D,nb_proc_in_dimension)]).reshape(-1)  
    WEIGHTS = np.array([W for _ in range(rank_dimension,D,nb_proc_in_dimension)]).reshape(-1) 
    rows = []
    columns = []
    for elm in list_pairs:
        index,sym_index = elm
